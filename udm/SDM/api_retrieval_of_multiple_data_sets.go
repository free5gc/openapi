...
type GetDataSetsRequest struct {
	Supi               *string
	DatasetNames       []models.Udm_SDM_DataSetName
...
func (r *GetDataSetsRequest) SetDatasetNames(
	DatasetNames []models.Udm_SDM_DataSetName,
) {
...
type GetDataSetsResponse struct {
	...
	Udm_SDM_SubscriptionDataSets *models.Udm_SDM_SubscriptionDataSets
}
...
func (a *RetrievalOfMultipleDataSetsApiService) GetDataSets(
	ctx context.Context,
	request *GetDataSetsRequest,
) (*GetDataSetsResponse, error) {
	var (
	...
	if request.DatasetNames == nil {
		return nil, openapi.ReportError("DatasetNames must be non nil")
	} else {
		if len(request.DatasetNames) < 2 {
			return &localVarReturnValue, openapi.ReportError("DatasetNames must have at least 2 elements")
		}
		err := openapi.AddQueryParams(&localVarQueryParams, "dataset-names", request.DatasetNames, "csv")
		if err != nil {
			return nil, err
		}
	}
	...
	case 200:
		localVarReturnValue.Udm_SDM_SubscriptionDataSets = new(
			models.Udm_SDM_SubscriptionDataSets,
		)
		err = openapi.Deserialize(
			localVarReturnValue.Udm_SDM_SubscriptionDataSets,
			localVarBody,
