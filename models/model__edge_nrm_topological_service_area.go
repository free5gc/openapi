/*
 * Nchf_ConvergedCharging
 *
 * ConvergedCharging Service    © 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved.
 *
 * Source file: 3GPP TS 32.291 V17.9.0: Telecommunication management; Charging management;  5G system, charging service; Stage 3.
 * Url: http://www.3gpp.org/ftp/Specs/archive/32_series/32.291/
 *
 * API version: 3.1.6
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package models

type _EdgeNrm_TopologicalServiceArea struct {
	CellIdList []int32 `json:"cellIdList,omitempty" yaml:"cellIdList,omitempty" bson:"cellIdList,omitempty"`

	TrackingAreaIdList []Nrmirp_GenericNrm_Tai `json:"trackingAreaIdList,omitempty" yaml:"trackingAreaIdList,omitempty" bson:"trackingAreaIdList,omitempty"`

	ServingPLMN *Nrmirp_ComDefs_PlmnId `json:"servingPLMN,omitempty" yaml:"servingPLMN,omitempty" bson:"servingPLMN,omitempty"`
}
