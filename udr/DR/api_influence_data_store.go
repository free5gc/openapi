...
func (r *ReadInfluenceDataRequest) SetInternalGroupIds(
	InternalGroupIds []string,
) {
...
func (a *InfluenceDataStoreApiService) ReadInfluenceData(
	ctx context.Context,
	request *ReadInfluenceDataRequest,
) (*ReadInfluenceDataResponse, error) {
	var (
	...
	if request.InfluenceIds != nil {
		if len(request.InfluenceIds) < 1 {
			return &localVarReturnValue, openapi.ReportError(
				"InfluenceIds must have at least 1 elements",
			)
		}
		err := openapi.AddQueryParams(
			&localVarQueryParams,
			"influence-Ids",
			request.InfluenceIds,
			"multi",
		)
		if err != nil {
			return nil, err
