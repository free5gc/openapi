...
func (r *ReadIPTVCongifurationDataRequest) SetInterGroupIds(
	InterGroupIds []string,
) {
...
func (a *IPTVConfigurationDataStoreApiService) ReadIPTVCongifurationData(
	ctx context.Context,
	request *ReadIPTVCongifurationDataRequest,
) (*ReadIPTVCongifurationDataResponse, error) {
	var (
	...
	if request.ConfigIds != nil {
		if len(request.ConfigIds) < 1 {
			return &localVarReturnValue, openapi.ReportError(
				"ConfigIds must have at least 1 elements",
			)
		}
	...
	if request.InterGroupIds != nil {
		if len(request.InterGroupIds) < 1 {
			return &localVarReturnValue, openapi.ReportError(
				"InterGroupIds must have at least 1 elements",
			)
		}
		err := openapi.AddQueryParams(
			&localVarQueryParams,
			"inter-group-ids",
			request.InterGroupIds,
			"multi",
		)
		if err != nil {
			return nil, err
